...
describe('create extension', function () {
  ...
  describe('with env defined inside the template different than the variants', () => {
    before(() => {
      helper.scopeHelper.setWorkspaceWithRemoteScope();
      // the variant env is a workspace env (and not e.g. teambit.react/react) because this branch
      // removed the react env from the core aspects.
      helper.env.setTsEnv();
      helper.command.create('component-generator', 'my-aspect', `--scope ${helper.scopes.remote}`);
    });
    it('should set the env according to the variant', () => {
      ...
      expect(env.json).to.equal(`${helper.scopes.remote}/ts-env`);
    });
  });
