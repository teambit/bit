...
`);
  ...
  // A new version can legitimately move a component's main file (e.g. it used to point into a
  // committed dist/ that a new env stopped emitting). A consumer whose .bitmap still records the old
  // main file, with that file gone from the disk, cannot load the component at all — checkout head
  // under a "theirs" resolution must write the new version anyway instead of dying on MainFileRemoved.
  describe('checkout head when the recorded main file was removed and the new version renamed it', () => {
    let beforeCheckout: string;
    before(() => {
      helper.scopeHelper.setWorkspaceWithRemoteScope();
      helper.fs.outputFile('comp1/index.js', barFooV1);
      // a second file, so removing the main file later leaves the component dir in place —
      // the MainFileRemoved shape, not the component-dir-deleted one
      helper.fs.outputFile('comp1/util.js', barFooV1);
      helper.command.addComponent('comp1');
      helper.command.tagAllWithoutBuild();
      helper.command.export();

      // A consumer workspace whose .bitmap records mainFile index.js.
      const consumerClone = helper.scopeHelper.cloneWorkspace();

      // The original workspace moves the main file to main.js in a new exported version.
      helper.fs.outputFile('comp1/main.js', barFooV2);
      helper.fs.deletePath('comp1/index.js');
      helper.command.runCmd(`bit add comp1 --main main.js --id comp1`);
      helper.command.tagAllWithoutBuild('--unmodified');
      helper.command.export();

      // Back on the consumer: the old main file was deleted on disk (as a git-side change would),
      // leaving .bitmap pointing at a file that no longer exists.
      helper.scopeHelper.getClonedWorkspace(consumerClone);
      helper.fs.deletePath('comp1/index.js');
      beforeCheckout = helper.scopeHelper.cloneWorkspace();
    });
    it('without a "theirs" resolution: still fails loudly on the unloadable component', () => {
      const output = helper.general.runWithTryCatch('bit checkout head --skip-dependency-installation');
      expect(output).to.have.string('main file');
      expect(output).to.have.string('was removed');
    });
    describe('with --auto-merge-resolve theirs', () => {
      let output: string;
      before(() => {
        helper.scopeHelper.getClonedWorkspace(beforeCheckout);
        output = helper.command.checkoutHead('--auto-merge-resolve theirs --skip-dependency-installation');
      });
      it('should check the new version out instead of failing', () => {
        expect(output).to.have.string('successfully');
        const bitMap = helper.bitMap.read();
        expect(bitMap.comp1.version).to.equal('0.0.2');
      });
      it('should write the new main file and heal the .bitmap mainFile record', () => {
        expect(path.join(helper.scopes.localPath, 'comp1/main.js')).to.be.a.file();
        const bitMap = helper.bitMap.read();
        expect(bitMap.comp1.mainFile).to.equal('main.js');
      });
      it('the component should be loadable again — no invalid components in bit status', () => {
        // not `expectStatusToNotHaveIssues`: the fixture skips dependency installation, which
        // legitimately reports a missing-packages issue. The claim here is only that the
        // MainFileRemoved invalidity is gone.
        const status = helper.command.statusJson();
        expect(status.invalidComponents).to.have.lengthOf(0);
      });
    });
