  ...
  function () {
    ...
    describe('using Yarn', () => {
      ...
      it('packageExtensions is taken into account when running install in the capsule', () => {
        ...
        const envFolder = `${helper.scopes.remote}_node-env-1@0.0.1`;
        expect(path.join(scopeAspectsCapsulesRootDir, envFolder, 'node_modules/is-positive')).to.be.a.path();
      });
    });
    describe('using pnpm', () => {
      before(() => {
      ...
      it('workspace .npmrc is taken into account when running install in the capsule', async () => {
        ...
        const modulesState = await readModulesManifest(
          path.join(scopeAspectsCapsulesRootDir, envFolder, 'node_modules')
        );
