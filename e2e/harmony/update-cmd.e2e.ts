...
describe('update command', function () {
  ...
  describe('updates all dependencies', function () {
    let configFile;
    let componentJson;
    before(() => {
  ...
  describe('updates only to compatible versions', function () {
    let configFile;
    let componentJson;
    before(() => {
      helper.scopeHelper.reInitLocalScopeHarmony();
      ...
      helper.componentJson.write(componentJson, 'comp2');
      helper.command.install('is-positive@3.0.0');
      helper.command.update('--yes --compatible-only');
      configFile = helper.bitJsonc.read(helper.scopes.localPath);
      componentJson = helper.componentJson.read('comp2');
    });
    it('should update the version range', function () {
      expect(configFile['teambit.dependencies/dependency-resolver'].policy.dependencies['is-positive']).equal(
        '3.0.0'
      );
    });
    it('should update the root dependency version in node_modules', function () {
