...
describe('getScopeBit', () => {
  it('should show a component when is running outside a scope', () => {
    ...
    sinon.stub(scope, 'loadScope').returns(Promise.reject(new ScopeNotFound()));
    sinon.stub(GlobalRemotes, 'load').returns(Promise.resolve({ toPlainObject: () => {}}));
    const showSpy = sinon.spy();
    const resolveStub = sinon.stub(Remotes.prototype, 'resolve').returns(Promise.resolve({show: showSpy}));
