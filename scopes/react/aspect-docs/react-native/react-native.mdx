...
This error can show up for multiple reason, and one of the most popular it's because we use a library in a React Native component that use Native API's.  
And the local Bit Dev Server run on Web, so it can't work properly with API's that suppose to be on a mobile device.  
...
One way to solve it is to see if the library have a support for Web and update your code to be also Web compatible.  
...
If for some reason a library don't have it's parallel library for the web and don't have a built in Websupport, you can't make the composition working on the local Bit dev server.

For example, [`react-native-svg`](https://github.com/react-native-svg/react-native-svg) library have a parallel library called [`react-native-svg-web`](https://github.com/bakerface/react-native-svg-web).  
Let's create a React Native extension and add a `webpack-transformers.ts` file in the extension under `webpack` folder, the file look like this:
...
In this case `react-native-svg-web` have these dependencies in the peer dependencies:
...
Now every components that will use the `react-native-svg` library will work fine with the compositions, and this solution can be used to every other library that use Native API's.
