...
React Native's environment transformers enable merging new configurations for different Bit aspects used by the React Native environment.
...
The example below shows React Native being applied on all components in the workspace, using the wildcard character `*`.
