...
To prevent such conflicts, set the `resolveFromEnv` property of that dependency as `true`. This will make sure to install the needed peer dependency in a directory set by the environment.
It will then resolve the path to that installed package for each component requiring it. Each component will only use the package relevant to its environment.
