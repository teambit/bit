...
workspace and capsule on the machine, so a dependency subgraph that was already installed once is
linked rather than re-created. Installs become significantly faster, at the cost of a virtual store
that is no longer contained in the workspace.
