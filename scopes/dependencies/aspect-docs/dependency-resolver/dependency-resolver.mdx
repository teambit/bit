...
By default, pnpm creates a symlinked `node_modules` wherease Yarn creates a "classic" hoisted one.
This may be changed by setting the `nodeLinker` configuration option (supported by pnpm only for now).

Allowed values of `nodeLinker`:

- `"hoisted"` - create a flat `node_modules` directory without using symlinks. Similar to the one created by npm or Yarn classic.
- `"isolated"` - create a `node_modules` directory with using symlinks. Similar to the one created by default by pnpm.
