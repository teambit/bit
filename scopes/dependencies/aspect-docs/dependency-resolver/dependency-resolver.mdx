The [Dependency Resolver](https://bit.cloud/teambit/dependencies/dependency-resolver) configures and installs dependencies for components in a Bit workspace. It auto-generates the dependency graph for components to spare us the tedious work of manually configuring it ourselves.
...
The Dependency resolver uses pnpm by default. Set the `packageManager` property to the ID of an aspect that registers
a `PackageManager` implementation with the Dependency resolver. Use `teambit.dependencies/pnpm` to configure the
built-in package manager explicitly.
...
By default, pnpm creates a symlinked `node_modules`. This may be changed by setting the `nodeLinker`
configuration option.

Allowed values of `nodeLinker`:

- `"hoisted"` - create a hoisted (aka flat) `node_modules` directory without using symlinks. Similar to the one created by npm.
- `"isolated"` - create a `node_modules` directory using symlinks. All dependencies are symlinked from a flat storage at `node_modules/.pnpm`.

### Share the dependency directories between workspaces

By default every workspace builds its own copy of the dependency directories under
`node_modules/.pnpm`. Setting `enableGlobalVirtualStore` moves them into a directory shared by every
workspace on the machine (pnpm's own `<storeDir>/links` unless overridden), so a dependency subgraph
that was already installed once is linked rather than re-created. Installs become significantly
faster, at the cost of a virtual store that is no longer contained in the workspace. Capsules are
not affected: they deliberately keep the project-local layout, whatever the workspace uses.
...
It can also be turned on for every workspace on the machine with
`bit config set enable_global_virtual_store true`, and pointed at a different location with
`globalVirtualStoreDir`. It has no effect when `nodeLinker` is `"hoisted"`, which does not use a
virtual store at all.

### Patch a dependency

`patchedDependencies` maps a package (optionally with a version range) to a patch file, applied to
the package right after it is extracted - the same setting pnpm reads from `pnpm-workspace.yaml`.
...
>
> <br />
> If your npm is configured to use a registry other than npmjs's, then the Dependency Resolver will use that configured
> registry, instead.
