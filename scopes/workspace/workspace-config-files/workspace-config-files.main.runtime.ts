...
import { formatSkippedOutput } from './outputs';
...
export type WriteConfigFilesOptions = {
  ...
  writers?: string[];
  /**
   * override config files that already exist in the workspace and were not generated by bit.
   * by default such files are left untouched, so the user's own configuration isn't lost.
   */
  force?: boolean;
};
...
export type WriteResults = {
  ...
  totalExtendingConfigFiles: number;
  /**
   * absolute paths of config files that already existed in the workspace and were not generated by bit,
   * therefore were not overridden.
   */
  skippedPaths: string[];
};
...
export class WorkspaceConfigFilesMain {
  ...
  /**
   * a human readable warning about config files bit didn't override because they were authored by the user.
   * returns an empty string when there are no such files.
   */
  formatSkippedFilesWarning(skippedPaths: string[] = []): string {
    if (!skippedPaths.length) return '';
    return formatSkippedOutput(skippedPaths, this.workspace.path);
  }
  ...
  private async write(envsExecutionContext: ExecutionContext[], opts: WriteConfigFilesOptions): Promise<WriteResults> {
    ...
    const results = await pMapSeries(
      ...
      async ([writerId, envEntries]) => {
        ...
        skippedPaths.push(...oneResult.skippedPaths);
        return oneResult;
      }
    );

    const totalWrittenFiles = totalRealConfigFiles + totalExtendingConfigFiles;
    return { writersResult: results, totalWrittenFiles, totalRealConfigFiles, totalExtendingConfigFiles, skippedPaths };
  }
  ...
  private async handleOneIdWriter(
    ...
    const { extendingConfigFiles: writtenExtendingConfigFiles, skippedPaths } = await handleExtendingConfigFiles(
      envEntries,
      envCompsDirsMap,
