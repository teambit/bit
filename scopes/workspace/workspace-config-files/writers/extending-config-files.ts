...
import type { ConfigWriterEntry, ExtendingConfigFile } from '../config-writer-entry';
...
export type HandleExtendingConfigFilesResult = {
  extendingConfigFiles: EnvsWrittenExtendingConfigFiles;
  /**
   * paths of config files that already exist in the workspace and were not generated by bit.
   * they are left untouched, otherwise the user's own configuration would be lost.
   */
  skippedPaths: string[];
};
...
export async function handleExtendingConfigFiles(
...
): Promise<HandleExtendingConfigFilesResult> {
  ...
  // all the entries here share the same writer-id (they're grouped by it before getting here), so any of them
  // knows how to tell whether an existing config file was generated by bit.
  const configWriter = envEntries[0]?.configWriter;
  return writeExtendingConfigFiles(extendingConfigFilesMap, fileHashPerDedupedPaths, workspaceDir, configWriter, opts);
}
...
/**
 * a config file that already exists in the workspace and wasn't generated by bit is a file the user wrote.
 * overriding it silently loses their configuration, so it's skipped unless the caller explicitly asked to force it.
 */
export async function isUserAuthoredConfigFile(filePath: string, configWriter?: ConfigWriterEntry): Promise<boolean> {
  // without isBitGenerated we have no way to tell the files apart, keep the original behavior of overriding.
  if (!configWriter?.isBitGenerated) return false;
  if (!(await fs.pathExists(filePath))) return false;
  try {
    return !configWriter.isBitGenerated(filePath);
  } catch {
    // the file exists but can't be read. better to leave a file we know nothing about as is.
    return true;
  }
}
...
async function writeExtendingConfigFiles(
  ...
  fileHashPerDedupedPaths: DedupedPaths,
  workspaceDir: string,
  configWriter: ConfigWriterEntry | undefined,
  opts: WriteConfigFilesOptions
): Promise<HandleExtendingConfigFilesResult> {
  const skippedPaths: string[] = [];
  const extendingConfigFiles: EnvsWrittenExtendingConfigFiles = await Promise.all(
      ...
      const writtenPaths = compact(
        await Promise.all(
          paths.map(async (path) => {
            ...
            // the check runs for dry-run as well, so it reports what will actually happen.
            if (!opts.force && (await isUserAuthoredConfigFile(filePath, configWriter))) {
  ...
  return { extendingConfigFiles, skippedPaths };
}
