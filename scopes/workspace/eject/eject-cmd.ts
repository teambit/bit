...
export class EjectCmd implements Command {
  ...
  async report([ids]: [string[]], { force, json }: { force: boolean; json: boolean }): Promise<string> {
    const bitIds = ids.map((id) => this.workspace.consumer.bitMap.getExistingBitId(id)); // this also assure that the ID is in .bitmap
    const componentEjector = new ComponentsEjector(this.workspace, this.logger, compact(bitIds), force);
