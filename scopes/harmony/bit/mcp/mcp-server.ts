...
export async function startMcpServer() {
  debug(`Starting mcp server. Process ID: ${process.pid}, CWD: ${process.cwd()}, Args: ${JSON.stringify(process.argv)}`);
  ...
  if (shouldRegisterTool("bit_status")) server.tool("bit_status",
    "Present the current status of components in the workspace, including indication of detected issues.",
    {
      ...cwdSchema,
      warnings: z.boolean().optional().describe("Show warnings. By default, only issues that block tag/snap are shown"),
      json: z.boolean().optional().describe("Return a JSON version of the component status"),
      verbose: z.boolean().optional().describe("Show extra data: full snap hashes for staged components, and divergence point for lanes"),
      ...
      ignoreCircularDependencies: z.boolean().optional().describe("Do not check for circular dependencies to get results quicker")
    },
    async ({ cwd, warnings, json, verbose, lanes, strict, ignoreCircularDependencies }: {
      cwd: string;
      warnings?: boolean;
      json?: boolean;
      verbose?: boolean;
  ...
  if (shouldRegisterTool("bit_list")) server.tool("bit_list",
    "List components on a workspace or a remote scope. Supports filtering, formatting, and highlighting outdated or deleted components.",
    {
      ...
      localScope: z.boolean().optional().describe("Show only components stored in the local scope, including indirect dependencies"),
      scope: z.string().optional().describe("Filter components by their scope name (e.g., teambit.workspace)"),
      outdated: z.boolean().optional().describe("Highlight outdated components compared to their latest remote version"),
      ...
      namespace: z.string().optional().describe("Filter components by their namespace (a logical grouping within a scope, e.g., 'ui', '*/ui')")
    },
    async ({ cwd, remoteScope, ids, localScope, scope, outdated, includeDeleted, json, namespace }: {
      cwd: string;
      remoteScope?: string;
      ids?: boolean;
      localScope?: boolean;
  ...
  if (shouldRegisterTool("bit_add")) server.tool("bit_add",
    "Track one or more directories as new components. Supports setting id, main file, namespace, scope, environment, and output format.",
    {
      ...
      scope: z.string().optional().describe("Set the component's scope. If not entered, the default-scope from workspace.jsonc will be used"),
      env: z.string().optional().describe("Set the component's environment. Overrides the env from variants if exists"),
      json: z.boolean().optional().describe("Output as json format")
    },
    async ({ cwd, path, id, main, namespace, override, scope, env, json }: {
      cwd: string;
      path: string[];
      id?: string;
      main?: string;
  ...
  if (shouldRegisterTool("bit_init")) server.tool("bit_init",
    "Create or reinitialize an empty workspace.",
    {
      ...
      generator: z.string().optional().describe("Add env-ids into the generators field in the workspace config for future 'bit create' templates (comma separated)"),
      standalone: z.boolean().optional().describe("Do not nest component store within .git directory and do not write config data inside package.json"),
      ...
      resetNew: z.boolean().optional().describe("Reset .bitmap file as if the components were newly added and remove all model data (objects)"),
      resetLaneNew: z.boolean().optional().describe("Same as reset-new, but only resets components belonging to lanes. Main components are left intact"),
      resetHard: z.boolean().optional().describe("Delete all Bit files and directories, including Bit configuration, tracking and model data. Useful for re-starting workspace from scratch"),
      resetScope: z.boolean().optional().describe("Removes local scope (.bit or .git/bit). Tags/snaps that have not been exported will be lost. Workspace is left intact"),
      defaultDirectory: z.string().optional().describe("Set the default directory pattern to import/create components into"),
    ...
    async ({ cwd, path, name, generator, standalone, noPackageJson, reset, resetNew, resetLaneNew, resetHard, resetScope, defaultDirectory, defaultScope, force, bare, shared }) => {
      const args = ["init"];
  ...
  if (shouldRegisterTool("bit_show")) server.tool("bit_show",
    "Display the component's essential information.",
    {
      ...
      compare: z.boolean().optional().describe("Legacy-only. Compare current file system component to its latest tagged version")
    },
  ...
  if (shouldRegisterTool("bit_tag")) server.tool("bit_tag",
    "Create an immutable and exportable component snapshot, tagged with a release version.",
    {
      ...cwdSchema,
      componentPatterns: z.array(z.string()).optional().describe("Component name, component id, or component pattern(s) to tag. By default, all new and modified are tagged."),
      message: z.string().optional().describe("A log message describing latest changes"),
      unmodified: z.boolean().optional().describe("Include unmodified components (by default, only new and modified components are tagged)"),
      editor: z.string().optional().describe("Open an editor to write a tag message for each component. Optionally, specify the editor-name (defaults to vim)."),
      ver: z.string().optional().describe("Tag with the given version"),
      increment: z.string().optional().describe("Increment level: major, premajor, minor, preminor, patch, prepatch, prerelease"),
      ...
      preRelease: z.string().optional().describe("Syntactic sugar for '--increment prerelease' and `--prerelease-id <identifier>`"),
      ...
      persist: z.string().optional().describe("Persist the changes generated by --soft tag. By default, run the build pipeline, unless 'skip-build' is provided"),
      ...
      rebuildDepsGraph: z.boolean().optional().describe("Do not reuse the saved dependencies graph, instead build it from scratch"),
      incrementBy: z.string().optional().describe("Increment semver flag (patch/minor/major) by. E.g. incrementing patch by 2: 0.0.1 -> 0.0.3."),
      ignoreNewestVersion: z.boolean().optional().describe("Allow tagging even when the component has newer versions e.g. for hotfixes."),
    ...
    async (params) => {
      ...
      for (const [key, value] of Object.entries(flags)) {
        if (typeof value === "boolean" && value) args.push(`--${key.replace(/[A-Z]/g, m => '-' + m.toLowerCase())}`);
        else if (typeof value === "string") args.push(`--${key.replace(/[A-Z]/g, m => '-' + m.toLowerCase())}`, value);
      }
      return runBit(cwd, args);
    }
  );

  if (shouldRegisterTool("bit_snap")) server.tool("bit_snap",
    "Create an immutable and exportable component snapshot (non-release version).",
    {
      ...cwdSchema,
      componentPattern: z.string().optional().describe("Component name, component id, or component pattern. By default, only new and modified components are snapped."),
      message: z.string().optional().describe("Snap message describing the latest changes - will appear in component history log"),
      unmodified: z.boolean().optional().describe("Include unmodified components (by default, only new and modified components are snapped)"),
      ...
      editor: z.string().optional().describe("Open an editor to write a snap message per component. Optionally specify the editor-name (defaults to vim)."),
      ...
      disableSnapPipeline: z.boolean().optional().describe("Skip the snap pipeline. This will for instance skip packing and publishing component version for install, and app deployment"),
  ...
  if (shouldRegisterTool("bit_import")) server.tool("bit_import",
    "Import components from their remote scopes to the local workspace.",
    {
      ...cwdSchema,
      componentPatterns: z.array(z.string()).optional().describe("Component IDs or component patterns (separated by space). Use patterns to import groups of components."),
      path: z.string().optional().describe("Import components into a specific directory (a relative path in the workspace)"),
      objects: z.boolean().optional().describe("Import components objects to the local scope without checkout (without writing them to the file system)"),
      ...
      skipDependencyInstallation: z.boolean().optional().describe("Do not auto-install dependencies of the imported components"),
      skipWriteConfigFiles: z.boolean().optional().describe("Do not write config files (such as eslint, tsconfig, prettier, etc...)"),
      merge: z.string().optional().describe("Merge local changes with the imported version. strategy should be 'theirs', 'ours' or 'manual'"),
      dependencies: z.boolean().optional().describe("Import all dependencies (bit components only) of imported components and write them to the workspace"),
      dependenciesHead: z.boolean().optional().describe("Same as --dependencies, except it imports the dependencies with their head version"),
      dependents: z.boolean().optional().describe("Import components found while traversing from the imported components upwards to the workspace components"),
      dependentsVia: z.string().optional().describe("Same as --dependents except the traversal must go through the specified component(s) (comma separated)"),
      dependentsAll: z.boolean().optional().describe("Same as --dependents except not prompting for selecting paths but rather selecting all paths and showing final confirmation before importing"),
      silent: z.boolean().optional().describe("No prompt for --dependents/--dependents-via flags"),
      filterEnvs: z.string().optional().describe("Only import components that have the specified environment (e.g., 'teambit.react/react-env')"),
      saveInLane: z.boolean().optional().describe("When checked out to a lane and the component is not on the remote-lane, save it in the lane (defaults to save on main)"),
      allHistory: z.boolean().optional().describe("Avoid optimizations, fetch all history versions, always"),
      fetchDeps: z.boolean().optional().describe("Fetch dependencies (bit components) objects to the local scope, but dont add to the workspace"),
      writeDeps: z.string().optional().describe("Write all workspace component dependencies to package.json or workspace.jsonc"),
      trackOnly: z.boolean().optional().describe("Do not write any component files, just create .bitmap entries of the imported components"),
      includeDeprecated: z.boolean().optional().describe("When importing with patterns, include deprecated components (default to exclude them)")
    },
    async (params) => {
      const { cwd, componentPatterns, ...flags } = params;
      const args = ["import"];
  ...
  if (shouldRegisterTool("bit_export")) server.tool("bit_export",
    "Export components from the workspace to remote scopes.",
    {
      ...cwdSchema,
      componentPatterns: z.array(z.string()).optional().describe("Component name, component id, or component pattern(s) to export. Not recommended to use patterns in most scenarios."),
      eject: z.boolean().optional().describe("After export, remove the components from the workspace and install them as packages"),
      ...
      originDirectly: z.boolean().optional().describe("Avoid export to the central hub, instead, export directly to the original scopes"),
      resume: z.string().optional().describe("In case the previous export failed and suggested to resume with an export-id, enter the id"),
      headOnly: z.boolean().optional().describe("In case previous export failed and locally it shows exported and only one snap/tag was created, try using this flag"),
      ignoreMissingArtifacts: z.boolean().optional().describe("Don't throw an error when artifact files are missing"),
      forkLaneNewScope: z.boolean().optional().describe("Allow exporting a forked lane into a different scope than the original scope"),
      openBrowser: z.boolean().optional().describe("Open a browser once the export is completed in the cloud job url"),
  ...
  if (shouldRegisterTool("bit_remove")) server.tool("bit_remove",
    "Remove component(s) from the local workspace.",
    {
      ...cwdSchema,
      componentPattern: z.string().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      track: z.boolean().optional().describe("Keep tracking component in .bitmap (default = false), helps transform a tagged-component to new"),
  ...
  if (shouldRegisterTool("bit_test")) server.tool("bit_test",
    "Test components in the workspace. By default only runs tests for new and modified components.",
    {
      ...cwdSchema,
      componentPattern: z.string().optional().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      ...
      updateSnapshot: z.boolean().optional().describe("If supported by the tester, re-record every snapshot that fails during the test run"),
  ...
  if (shouldRegisterTool("bit_checkout")) server.tool("bit_checkout",
    "Switch between component versions or remove local changes.",
    {
      ...cwdSchema,
      to: z.string().describe("Permitted values: [head, latest, reset, {specific-version}, {head~x}]. 'head' - last snap/tag. 'latest' - semver latest tag. 'reset' - removes local changes"),
      componentPattern: z.string().optional().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      interactiveMerge: z.boolean().optional().describe("When a component is modified and the merge process found conflicts, display options to resolve them"),
      autoMergeResolve: z.string().optional().describe("In case of merge conflict, resolve according to the provided strategy: [ours, theirs, manual]"),
      manual: z.boolean().optional().describe("Same as --auto-merge-resolve manual. In case of merge conflict, write the files with the conflict markers"),
      all: z.boolean().optional().describe("All components"),
      workspaceOnly: z.boolean().optional().describe("Only relevant for 'bit checkout head' when on a lane. Don't import components from the remote lane that are not already in the workspace"),
  ...
  if (shouldRegisterTool("bit_merge")) server.tool("bit_merge",
    "Merge changes of the remote head into local - auto-snaps all merged components.",
    {
      ...cwdSchema,
      componentPattern: z.string().optional().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      autoMergeResolve: z.string().optional().describe("In case of a conflict, resolve according to the strategy: [ours, theirs, manual]"),
      manual: z.boolean().optional().describe("Same as --auto-merge-resolve manual. In case of merge conflict, write the files with the conflict markers"),
      abort: z.boolean().optional().describe("In case of an unresolved merge, revert to pre-merge state"),
      resolve: z.boolean().optional().describe("Mark an unresolved merge as resolved and create a new snap with the changes"),
      noSnap: z.boolean().optional().describe("Do not auto snap even if the merge completed without conflicts"),
      build: z.boolean().optional().describe("In case of snap during the merge, run the build-pipeline (similar to bit snap --build)"),
      verbose: z.boolean().optional().describe("Show details of components that were not merged successfully"),
      skipDependencyInstallation: z.boolean().optional().describe("Do not install new dependencies resulting from the merge"),
  ...
  if (shouldRegisterTool("bit_build")) server.tool("bit_build",
    "Run set of tasks for build. By default, only new and modified components are built.",
    {
      ...cwdSchema,
      componentPattern: z.string().optional().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      unmodified: z.boolean().optional().describe("Include unmodified components (by default, only new and modified components are built)"),
      ...
      reuseCapsules: z.boolean().optional().describe("Avoid deleting the capsules root-dir before starting the build"),
      ...
      cachePackagesOnCapsuleRoot: z.boolean().optional().describe("Set the package-manager cache on the capsule root"),
      listTasks: z.string().optional().describe("List tasks of an env or a component-id for each one of the pipelines: build, tag and snap"),
      skipTests: z.boolean().optional().describe("Skip running component tests during build process"),
      skipTasks: z.string().optional().describe("Skip the given tasks (comma separated)"),
      failFast: z.boolean().optional().describe("Stop pipeline execution on the first failed task"),
      includeSnap: z.boolean().optional().describe("Include snap pipeline tasks. Warning: this may deploy/publish if you have such tasks"),
      includeTag: z.boolean().optional().describe("Include tag pipeline tasks. Warning: this may deploy/publish if you have such tasks"),
  ...
  if (shouldRegisterTool("bit_lint")) server.tool("bit_lint",
  ...
  if (shouldRegisterTool("bit_format")) server.tool("bit_format",
    "Format components in the development workspace.",
    {
      ...
      check: z.boolean().optional().describe("Output a human-friendly message and a list of unformatted files, if any"),
  ...
  if (shouldRegisterTool("bit_diff")) server.tool("bit_diff",
    "Show the diff between the components' current source files and config, and their latest snapshot or tag.",
    {
      ...cwdSchema,
      componentPattern: z.string().optional().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      version: z.string().optional().describe("The base version to compare from. If omitted, compares the workspace's current files to the component's latest version."),
      toVersion: z.string().optional().describe("The target version to compare against 'version'. If both 'version' and 'to-version' are provided, compare those two versions directly."),
      parent: z.boolean().optional().describe("Compare the specified 'version' to its immediate parent instead of comparing to the current one"),
  ...
  if (shouldRegisterTool("bit_log")) server.tool("bit_log",
    "Show components(s) version history.",
    {
      ...
      fullHash: z.boolean().optional().describe("Show full hash of the snap (default to the first 9 characters for --one-line/--parents flags)"),
      fullMessage: z.boolean().optional().describe("Show full message of the snap (default to the first line for --one-line/--parents flags)"),
      showHidden: z.boolean().optional().describe("Show hidden snaps (snaps are marked as hidden typically when the following tag has the same files/config)"),
  ...
  if (shouldRegisterTool("bit_remove_component")) server.tool("bit_remove_component",
    "Remove component(s) from the local workspace.",
    {
      ...cwdSchema,
      componentPattern: z.string().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
  ...
  if (shouldRegisterTool("bit_watch")) server.tool("bit_watch",
    "Automatically recompile modified components (on save).",
    {
      ...
      import: z.boolean().optional().describe("Import component objects if .bitmap changed not by bit (DEPRECATED, now default)"),
      skipImport: z.boolean().optional().describe("Do not import component objects if .bitmap changed not by bit"),
      generateTypes: z.boolean().optional().describe("EXPERIMENTAL. Generate d.ts files for typescript components (hurts performance)"),
      trigger: z.string().optional().describe("Trigger recompilation of the specified component regardless of what changed")
    },
    async (params) => {
  ...
  if (shouldRegisterTool("bit_envs")) server.tool("bit_envs",
  ...
  if (shouldRegisterTool("bit_envs_list")) server.tool("bit_envs_list",
  ...
  if (shouldRegisterTool("bit_envs_get")) server.tool("bit_envs_get",
    "Show config information from a component's env.",
    {
      ...cwdSchema,
      componentName: z.string().describe("The 'component name' or 'component id' of the component whose env you'd like to inspect"),
      services: z.string().optional().describe("Show information about the specific services only. For multiple services, separate by a comma and wrap with quotes")
    },
  ...
  if (shouldRegisterTool("bit_envs_list_core")) server.tool("bit_envs_list_core",
  ...
  if (shouldRegisterTool("bit_envs_set")) server.tool("bit_envs_set",
    "Set envs for components.",
    {
      ...cwdSchema,
      pattern: z.string().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      envId: z.string().describe("The env's component id"),
      config: z.string().optional().describe("The env config. Enter the config as a stringified JSON (e.g. '{\"foo\":\"bar\"}')")
    },
  ...
  if (shouldRegisterTool("bit_envs_unset")) server.tool("bit_envs_unset",
  ...
  if (shouldRegisterTool("bit_envs_update")) server.tool("bit_envs_update",
    "Update a version of an env for all or specified components.",
    {
      ...cwdSchema,
      envId: z.string().describe("The env's component id. Optionally, add a version (id@version), otherwise will use the latest version from the remote"),
      pattern: z.string().optional().describe("The components to update (defaults to all components). Component name, component id, or component pattern.")
    },
  ...
  if (shouldRegisterTool("bit_config_set")) server.tool("bit_config_set",
    "Set a configuration. Defaults to save it globally.",
    {
      ...
      localTrack: z.boolean().optional().describe("Set the configuration in the current workspace (saved in workspace.jsonc)")
    },
  ...
  if (shouldRegisterTool("bit_config_del")) server.tool("bit_config_del",
  ...
  if (shouldRegisterTool("bit_config_get")) server.tool("bit_config_get",
  ...
  if (shouldRegisterTool("bit_config_list")) server.tool("bit_config_list",
  ...
  if (shouldRegisterTool("bit_capsule_list")) server.tool("bit_capsule_list",
  ...
  if (shouldRegisterTool("bit_capsule_create")) server.tool("bit_capsule_create",
    "Create capsules for components.",
    {
      ...
      useHash: z.boolean().optional().describe("Whether to use hash function (of base dir) as capsules root dir name"),
      json: z.boolean().optional().describe("Json format"),
    ...
    async ({ cwd, componentIds, baseDir, rootBaseDir, alwaysNew, seedersOnly, id, useHash, json, installPackages, packageManager }) => {
  ...
  if (shouldRegisterTool("bit_capsule_delete")) server.tool("bit_capsule_delete",
  ...
  if (shouldRegisterTool("bit_remote_add")) server.tool("bit_remote_add",
  ...
  if (shouldRegisterTool("bit_remote_del")) server.tool("bit_remote_del",
  ...
  if (shouldRegisterTool("bit_help")) server.tool("bit_help",
    "Shows help.",
    {
      ...cwdSchema,
  ...
  if (shouldRegisterTool("bit_version")) server.tool("bit_version",
    "Shows bit version.",
    {
      ...cwdSchema,
  ...
  if (shouldRegisterTool("bit_whoami")) server.tool("bit_whoami",
  ...
  if (shouldRegisterTool("bit_logout")) server.tool("bit_logout",
  ...
  if (shouldRegisterTool("bit_init_bare")) server.tool("bit_init_bare",
  ...
  if (shouldRegisterTool("bit_new")) server.tool("bit_new",
    "Create a new workspace from a template.",
    {
      ...cwdSchema,
      templateName: z.string().describe("The name of the workspace template"),
      workspaceName: z.string().describe("The name for the new workspace and workspace directory that will be created"),
      ...
      defaultScope: z.string().optional().describe("Set the default scope for the workspace. Used in the generated workspace.jsonc"),
      skipGit: z.boolean().optional().describe("Skip generation of Git repository in the new workspace"),
      empty: z.boolean().optional().describe("Skip template's default component creation (relevant for templates that add components by default)"),
      loadFrom: z.string().optional().describe("Local path to the workspace containing the template"),
      currentDir: z.boolean().optional().describe("Create the new workspace in current directory (default is to create a new directory, inside the current dir)")
    },
    async ({ cwd, templateName, workspaceName, aspect, template, env, defaultScope, skipGit, empty, loadFrom, currentDir }) => {
  ...
  if (shouldRegisterTool("bit_deprecate")) server.tool("bit_deprecate",
  ...
  if (shouldRegisterTool("bit_undeprecate")) server.tool("bit_undeprecate",
  ...
  if (shouldRegisterTool("bit_move")) server.tool("bit_move",
    "Move a component to a different filesystem path.",
    {
      ...
      newComponentDir: z.string().describe("The new directory (relative to the workspace root) to create and move the component's files to")
    },
  ...
  if (shouldRegisterTool("bit_link")) server.tool("bit_link",
    "Create links in the node_modules directory, to core aspects and to components in the workspace.",
    {
      ...
      target: z.string().optional().describe("Link to an external directory (similar to npm-link) so other projects could use these components"),
  ...
  if (shouldRegisterTool("bit_lanes")) server.tool("bit_lanes",
  ...
  if (shouldRegisterTool("bit_lane_show")) server.tool("bit_lane_show",
  ...
  if (shouldRegisterTool("bit_lane_create")) server.tool("bit_lane_create",
    "Creates a new lane and switches to it.",
    {
      ...
      forkLaneNewScope: z.boolean().optional().describe("Create the new lane in a different scope than its parent lane")
    },
  ...
  if (shouldRegisterTool("bit_lane_remove")) server.tool("bit_lane_remove",
  ...
  if (shouldRegisterTool("bit_lane_rename")) server.tool("bit_lane_rename",
    "Change the lane-name locally.",
    {
      ...cwdSchema,
      newName: z.string().describe("The new lane name"),
      laneName: z.string().optional().describe("The name of the lane to rename. if not specified, the current lane is used")
    },
  ...
  if (shouldRegisterTool("bit_lane_alias")) server.tool("bit_lane_alias",
  ...
  if (shouldRegisterTool("bit_lane_diff")) server.tool("bit_lane_diff",
    "Show diff between lanes.",
    {
      ...
      pattern: z.string().optional().describe("Show lane-diff for components conforming to the specified component-pattern only")
    },
  ...
  if (shouldRegisterTool("bit_lane_switch")) server.tool("bit_lane_switch",
    "Switch to the specified lane.",
    {
      ...cwdSchema,
      lane: z.string().describe("Lane-name or lane-id (if lane is not local) to switch to"),
      head: z.boolean().optional().describe("Switch to the head of the lane/main (fetches the latest changes from the remote)"),
      autoMergeResolve: z.string().optional().describe("Merge local changes with the checked out version. strategy should be 'theirs', 'ours' or 'manual'"),
      forceOurs: z.boolean().optional().describe("Do not merge, preserve local files as is"),
      forceTheirs: z.boolean().optional().describe("Do not merge, just overwrite with incoming files"),
      workspaceOnly: z.boolean().optional().describe("Checkout only the components in the workspace to the selected lane"),
      skipDependencyInstallation: z.boolean().optional().describe("Do not install dependencies of the imported components"),
      pattern: z.string().optional().describe("Switch only the lane components matching the specified component-pattern. only works when the workspace is empty"),
      alias: z.string().optional().describe("Relevant when the specified lane is a remote lane. create a local alias for the lane"),
      verbose: z.boolean().optional().describe("Display detailed information about components that legitimately were not switched"),
      json: z.boolean().optional().describe("Return the output as JSON")
    },
    async ({ cwd, lane, head, autoMergeResolve, forceOurs, forceTheirs, workspaceOnly, skipDependencyInstallation, pattern, alias, verbose, json }) => {
  ...
  if (shouldRegisterTool("bit_fetch")) server.tool("bit_fetch",
    "Fetch remote objects and store locally.",
    {
      ...
      allHistory: z.boolean().optional().describe("For each component, fetch all its versions. By default, only the latest version is fetched"),
      json: z.boolean().optional().describe("Return the output as JSON"),
      fromOriginalScopes: z.boolean().optional().describe("Fetch indirect dependencies from their original scope as opposed to from their dependents")
    },
  ...
  if (shouldRegisterTool("bit_install")) server.tool("bit_install",
    "Installs workspace dependencies.",
    {
      ...
      update: z.boolean().optional().describe("Update all dependencies to latest version according to their semver range"),
      ...
      skipUnavailable: z.boolean().optional().describe("When adding missing dependencies, skip those that are not found in the registry"),
      addMissingPeers: z.boolean().optional().describe("Install all missing peer dependencies"),
      recurringInstall: z.boolean().optional().describe("Automatically run install again if there are non loaded old envs in your workspace"),
      noOptional: z.boolean().optional().describe("Do not install optional dependencies (works with pnpm only)"),
      lockfileOnly: z.boolean().optional().describe("Dependencies are not written to node_modules. Only the lockfile is updated")
    },
    async (params) => {
  ...
  if (shouldRegisterTool("bit_uninstall")) server.tool("bit_uninstall",
  ...
  if (shouldRegisterTool("bit_update")) server.tool("bit_update",
    "Update dependencies. By default, dependencies are updated to the highest semver compatible versions.",
    {
      ...cwdSchema,
      packagePatterns: z.array(z.string()).optional().describe("A string list of package names, or patterns (separated by spaces or commas)"),
      yes: z.boolean().optional().describe("Automatically update all outdated versions for packages specified in pattern (all if no pattern supplied)"),
  ...
  if (shouldRegisterTool("bit_recover")) server.tool("bit_recover",
    "Recover component(s) soft-deleted from the workspace, or a remote scope.",
    {
      ...cwdSchema,
      componentName: z.string().describe("Component name or component id"),
      skipDependencyInstallation: z.boolean().optional().describe("Do not install packages in case of importing components"),
      skipWriteConfigFiles: z.boolean().optional().describe("Do not write config files (such as eslint, tsconfig, prettier, etc...)")
    },
  ...
  if (shouldRegisterTool("bit_clear_cache")) server.tool("bit_clear_cache",
  ...
  if (shouldRegisterTool("bit_create")) server.tool("bit_create",
    "Create a new component (source files and config) using a template.",
    {
      ...cwdSchema,
      templateName: z.string().describe("The template for generating the component (run 'bit templates' for a list of available templates)"),
      ...
      scope: z.string().optional().describe("Sets the component's scope-name. if not entered, the default-scope will be used"),
      aspect: z.string().optional().describe("Aspect-id of the template. helpful when multiple aspects use the same template name"),
      template: z.string().optional().describe("Env-id of the template. alias for --aspect."),
      path: z.string().optional().describe("Relative path in the workspace. by default the path is <scope>/<namespace>/<name>"),
      env: z.string().optional().describe("Set the component's environment. (overrides the env from variants and the template)"),
  ...
  if (shouldRegisterTool("bit_lane_merge")) server.tool("bit_lane_merge",
    "Merge a local or a remote lane to the current lane.",
    {
      ...
      pattern: z.string().optional().describe("Partially merge the lane - only components that match the specified component-pattern"),
      manual: z.boolean().optional().describe("Same as --auto-merge-resolve manual. In case of merge conflict, write the files with the conflict markers"),
      autoMergeResolve: z.string().optional().describe("In case of a merge conflict, resolve according to the strategy: [ours, theirs, manual]"),
      ours: z.boolean().optional().describe("DEPRECATED. use --auto-merge-resolve. in case of a conflict, keep local modifications"),
      theirs: z.boolean().optional().describe("DEPRECATED. use --auto-merge-resolve. in case of a conflict, override the local modification with the specified version"),
      workspace: z.boolean().optional().describe("Merge only lane components that are in the current workspace"),
      noAutoSnap: z.boolean().optional().describe("Do not auto snap after merge completed without conflicts of diverged components (see command description)"),
      noSnap: z.boolean().optional().describe("Do not pass snaps from the other lane even for non-diverged components (see command description)"),
      tag: z.boolean().optional().describe("Auto-tag all lane components after merging into main (or tag-merge in case of snap-merge)"),
      build: z.boolean().optional().describe("In case of snap during the merge, run the build-pipeline (similar to bit snap --build)"),
      message: z.string().optional().describe("Override the default message for the auto snap"),
      keepReadme: z.boolean().optional().describe("Skip deleting the lane readme component after merging"),
      noSquash: z.boolean().optional().describe("Relevant for merging lanes into main, which by default squashes all lane snaps"),
      squash: z.boolean().optional().describe("Relevant for merging a lane into another non-main lane, which by default does not squash"),
      ignoreConfigChanges: z.boolean().optional().describe("Allow merging when components are modified due to config changes (such as dependencies) only and not files"),
      verbose: z.boolean().optional().describe("Display detailed information about components that were legitimately unmerged"),
      skipDependencyInstallation: z.boolean().optional().describe("Do not install dependencies of the imported components"),
      skipFetch: z.boolean().optional().describe("Use the local state of target-lane if exits locally, without updating it from the remote"),
      includeDeps: z.boolean().optional().describe("Merge also dependencies of the specified components"),
      resolveUnrelated: z.string().optional().describe("Relevant when a component on a lane and the component on main have nothing in common. merge-strategy can be 'ours' (default) or 'theirs'"),
      excludeNonLaneComps: z.boolean().optional().describe("When merging main into a lane, exclude workspace components that are not on the lane (by default all workspace components are merged)")
    },
    async ({ cwd, lane, pattern, manual, autoMergeResolve, ours, theirs, workspace, noAutoSnap, noSnap, tag, build, message, keepReadme, noSquash, squash, ignoreConfigChanges, verbose, skipDependencyInstallation, skipFetch, includeDeps, resolveUnrelated, excludeNonLaneComps }) => {
  ...
  if (shouldRegisterTool("bit_templates")) server.tool("bit_templates",
  ...
  if (shouldRegisterTool("bit_reset")) server.tool("bit_reset",
    "Revert tagged or snapped versions for component(s).",
    {
      ...
      head: z.boolean().optional().describe("Revert the head tag/snap only (by default, all local tags/snaps are reverted)"),
      soft: z.boolean().optional().describe("Revert only soft-tags (components tagged with --soft flag)"),
      force: z.boolean().optional().describe("Revert the tag even if it's used as a dependency. WARNING: components that depend on this tag will be corrupted"),
  ...
  if (shouldRegisterTool("bit_fork")) server.tool("bit_fork",
    "Create a new component forked from an existing one (copies source files and configs).",
    {
      ...
      targetComponentName: z.string().optional().describe("The name for the new component (component name without scope, e.g. name/spaces/my-button). To set a different scope, use the '--scope' flag"),
      scope: z.string().optional().describe("Default scope for the new component"),
      path: z.string().optional().describe("Relative path in the workspace for the new component. By default the path is <scope>/<namespace>/<name>"),
      refactor: z.boolean().optional().describe("Update the import/require statements in all dependent components (in the same workspace)"),
      skipDependencyInstallation: z.boolean().optional().describe("Do not install packages of the imported components"),
      env: z.string().optional().describe("Set the environment for the new component"),
      skipConfig: z.boolean().optional().describe("Do not copy the config (aspects-config, env, etc) to the new component. Helpful when it fails during aspect loading"),
      preserve: z.boolean().optional().describe("Avoid refactoring file and variable/class names according to the new component name"),
    ...
    async ({ cwd, sourceComponentId, targetComponentName, scope, path, refactor, skipDependencyInstallation, env, skipConfig, preserve, noLink, ast }) => {
  ...
  if (shouldRegisterTool("bit_rename")) server.tool("bit_rename",
    "Rename component. If exported, create a new component and delete the original component. Otherwise just renames current component.",
    {
      ...
      newName: z.string().describe("The new component name (without its scope name. use --scope to define the new component's scope)"),
      scope: z.string().optional().describe("Define the scope for the new component"),
      refactor: z.boolean().optional().describe("Update the import/require statements in all dependent components (in the same workspace)"),
      preserve: z.boolean().optional().describe("Avoid renaming files and variables/classes according to the new component name"),
      ast: z.boolean().optional().describe("Use ast to transform files instead of regex"),
  ...
  if (shouldRegisterTool("bit_dependents")) server.tool("bit_dependents",
  ...
  if (shouldRegisterTool("bit_compile")) server.tool("bit_compile",
    "Compile components in the workspace.",
    {
      ...cwdSchema,
      componentNames: z.array(z.string()).optional().describe("A list of component names or component IDs (defaults to all components)"),
  ...
  if (shouldRegisterTool("bit_ws_config")) server.tool("bit_ws_config",
  ...
  if (shouldRegisterTool("bit_stash")) server.tool("bit_stash",
  ...
  if (shouldRegisterTool("bit_schema")) server.tool("bit_schema",
  ...
  if (shouldRegisterTool("bit_check_types")) server.tool("bit_check_types",
  ...
  if (shouldRegisterTool("bit_aspect")) server.tool("bit_aspect",
    "Manage aspects (list, list-core, get, set, unset, update).",
    {
      ...cwdSchema,
      subCommand: z.enum(["list", "list-core", "get", "set", "unset", "update"]).describe("Sub-command: list, list-core, get, set, unset, update"),
      pattern: z.string().optional().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      aspectId: z.string().optional().describe("The aspect's component id"),
      config: z.string().optional().describe("The aspect config. Enter the config as a stringified JSON (for set sub-command)"),
      merge: z.boolean().optional().describe("Merge with an existing config if exists (for set sub-command)"),
      debug: z.boolean().optional().describe("Show the origins where the aspects were taken from (for list/get sub-commands)"),
  ...
  if (shouldRegisterTool("bit_refactor")) server.tool("bit_refactor",
  ...
  if (shouldRegisterTool("bit_why")) server.tool("bit_why",
  ...
  if (shouldRegisterTool("bit_app")) server.tool("bit_app",
  ...
  if (shouldRegisterTool("bit_insight")) server.tool("bit_insight",
  ...
  if (shouldRegisterTool("bit_deps")) server.tool("bit_deps",
    "Manage dependencies.",
    {
      ...cwdSchema,
  ...
  if (shouldRegisterTool("bit_log_file")) server.tool("bit_log_file",
  ...
  if (shouldRegisterTool("bit_blame")) server.tool("bit_blame",
  ...
  if (shouldRegisterTool("bit_scope")) server.tool("bit_scope",
  ...
  if (shouldRegisterTool("bit_deps_get")) server.tool("bit_deps_get",
  ...
  if (shouldRegisterTool("bit_deps_remove")) server.tool("bit_deps_remove",
  ...
  if (shouldRegisterTool("bit_deps_unset")) server.tool("bit_deps_unset",
  ...
  if (shouldRegisterTool("bit_deps_debug")) server.tool("bit_deps_debug",
  ...
  if (shouldRegisterTool("bit_deps_set")) server.tool("bit_deps_set",
  ...
  if (shouldRegisterTool("bit_deps_reset")) server.tool("bit_deps_reset",
  ...
  if (shouldRegisterTool("bit_deps_eject")) server.tool("bit_deps_eject",
  ...
  if (shouldRegisterTool("bit_deps_blame")) server.tool("bit_deps_blame",
  ...
  if (shouldRegisterTool("bit_deps_usage")) server.tool("bit_deps_usage",
    "Find components that use the specified dependency.",
    {
      ...cwdSchema,
      dependency: z.string().describe("Dependency name")
  ...
  if (shouldRegisterTool("bit_deps_write")) server.tool("bit_deps_write",
  ...
  if (shouldRegisterTool("bit_artifacts")) server.tool("bit_artifacts",
  ...
  if (shouldRegisterTool("bit_globals")) server.tool("bit_globals",
    "List all globals.",
    {
      ...cwdSchema,
      json: z.boolean().optional().describe("Return the output as JSON")
  ...
  if (shouldRegisterTool("bit_system")) server.tool("bit_system",
    "System operations.",
    {
      ...cwdSchema,
  ...
  if (shouldRegisterTool("bit_eject")) server.tool("bit_eject",
    "Remove component from the workspace and install it instead as a regular npm package.",
    {
      ...cwdSchema,
      componentPattern: z.string().describe("Component name, component id, or component pattern. Use component pattern to select multiple components."),
      force: z.boolean().optional().describe("Eject even if the component is used as a dependency by other components in the workspace"),
  ...
  if (shouldRegisterTool("bit_lane_change_scope")) server.tool("bit_lane_change_scope",
  ...
  if (shouldRegisterTool("bit_lane_import")) server.tool("bit_lane_import",
  ...
  if (shouldRegisterTool("bit_lane_fetch")) server.tool("bit_lane_fetch",
  ...
  if (shouldRegisterTool("bit_lane_eject")) server.tool("bit_lane_eject",
  ...
  if (shouldRegisterTool("bit_lane_history")) server.tool("bit_lane_history",
  ...
  if (shouldRegisterTool("bit_lane_history_diff")) server.tool("bit_lane_history_diff",
  ...
  if (shouldRegisterTool("bit_lane_checkout")) server.tool("bit_lane_checkout",
  ...
  if (shouldRegisterTool("bit_lane_revert")) server.tool("bit_lane_revert",
  ...
  if (shouldRegisterTool("bit_lane_merge_abort")) server.tool("bit_lane_merge_abort",
  ...
  if (shouldRegisterTool("bit_lane_merge_move")) server.tool("bit_lane_merge_move",
  ...
  if (shouldRegisterTool("bit_pattern")) server.tool("bit_pattern",
