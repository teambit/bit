...
import { LRUCacheAdapter, DEFAULT_ENTRY_SIZE } from './lru-cache-adapter';
...
describe('LRUCacheAdapter', () => {
  ...
  describe('bounded by bytes', () => {
    it('should keep many small entries as long as their total size fits', () => {
      const cache = new LRUCacheAdapter<string>({ maxBytes: 10_000 });
    ...
    it('should evict the least recently used entries once the total size exceeds the limit', () => {
      const cache = new LRUCacheAdapter<string>({ maxBytes: 100 });
      cache.set('a', 'a', 40);
      cache.set('b', 'b', 40);
