...
a new bare-scope needs to be created and it will import the components to this scope first
...
| `--original-scope` |                  | sign components from the original scope. works only when all components are from the same scope                                                                                                             |
| `--save-locally`   |                  | save the signed components locally on the bare-scope for debugging purposes                                                                                                                                 |
| `--loose`          |                  | allow sign to succeed even if tasks like tests or lint fail                                                                                                                                                 |
| `--reuse-capsules` |                  | avoid deleting the capsules root-dir before starting the build                                                                                                                                              |
| `--tasks <string>` |                  | build the specified task(s) only. for multiple tasks, separate by a comma and wrap with quotes. specify the task-name (e.g. "TypescriptCompiler") or the task-aspect-id (e.g. teambit.compilation/compiler) |
