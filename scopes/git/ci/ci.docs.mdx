...
3. **Authorship** — for a live branch, the tip must carry the `[bit-sync]` marker on its own line. A
   ...
   deletion, and it can never authorize one.
...
`bit ci sync` consumes it. A main run tags the drifted components (patch bump)
with the message `align dependency context`, and exports. A lane run never
snaps drifted components; it snaps only the components with git-authored
changes and reports the drift. Pin the engine in `workspace.jsonc`
