...
| `--all`     | Reconcile every mapped lane plus the main scope.                                                     |
| `--main`    | Reconcile only the main scope against the default branch (opens a sync PR on drift).                 |
| `--dry-run` | Report the action that _would_ be taken per target; nothing is committed, pushed, or sent to GitHub. |
...
1. `git checkout -B <mainSyncBranch>` from the existing sync branch, or from the default branch if
   there isn't one — then merge the default branch in, so the diff is computed against current `main`.
2. `bit checkout head` (including components that exist on the scope but not yet in this workspace).
...
| --------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `mode`                | `git-source-of-truth` | `git-source-of-truth`: merges happen in GitHub, lanes mirror to PRs. `mirror`: bit.cloud merges are allowed and git tracks them.    |
