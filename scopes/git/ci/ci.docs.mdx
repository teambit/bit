...
| `--auto-tag-increment` |           | Version bump level for auto-tagged dependents (default: `patch`). See below.                                        |

### Bumping Dependents

The version flags above apply only to the components that actually changed. Dependents that are
picked up by auto-tag are always bumped by a `patch`, regardless of `--major`/`--minor`:
...
Use `--auto-tag-increment` to bump the dependents by a different level:
