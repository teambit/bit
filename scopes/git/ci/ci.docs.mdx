...
The command snaps the tree of the branch locally, without exporting, to check that first. Two outcomes:

- **The snap is a no-op.** The content of the branch already matches the lane's — the common shape, since
...
- **The snap would change something.** The two sides genuinely disagree, with no known common history to
  merge from. The command halts, the same way it halted before this action existed: it cannot tell which
  side is newer. Nothing is exported; the lane is untouched.

Adoption never touches a branch whose `.bitmap` already names a DIFFERENT lane with a live, unmerged claim.
A different-lane pointer that is already merged into the default branch — the ordinary shape of a branch cut
after some other lane's sync pull request landed — is history, not a claim, and does not block adoption.
...
repositories the change touches. The two are different. This version supports one shape only: the content of
the lane must live in one scope, the `defaultScope` of this workspace. The hosting scope can be another
scope, and a scope-qualified id addresses such a lane.
...
**The command never mirrors a lane that has components outside `defaultScope`.** Such a mirror writes the
components of another repository into this repository, and exports them from here. A lane like this is still
legitimate, so only the report depends on how the command reached the lane:
...
| **The lane became cross-scope during a live mirror**                 | `HALTED`  | ≠ 0  | yes — labelled and commented |

The skip keeps the exit code at 0 on purpose: a permanent cross-scope lane must not fail every scheduled run.
