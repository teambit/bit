...
export class CiMain {
  ...
  /** Config-only sync from main onto the lane; see `sync/main-config-sync.ts`. */
  private async syncConfigFromMain(laneId: LaneId) {
    await syncConfigFromMainImpl(
  ...
  /** The bit binary this process runs — compared against a drifted component's `recordedBitVersion`. */
  getRunningBitVersion(): string {
    return getBitVersion();
  }

  /**
   * Split the tag-pending set into git-authored changes and dependency-context drift.
   * See `sync/context-drift-detector.ts` for what counts as drift.
   */
