...
New issue class (`LoadFailures`) in `@teambit/component-issues`, carrying `{ failedId, phase, error }` entries, attached via the existing `ConsumerComponent.issues` mechanism that `bit status` already renders.

- **Only report at _terminal_ failure sites.** Bit's load is intentionally fault-tolerant and multi-pass: an aspect that fails at one stage is frequently loaded successfully at a later stage (e.g. on-the-fly compilation, then re-require). Reporting at a catch site that a later pass recovers from would be a false positive. We therefore only surface failures from sites proven terminal-for-the-command:
  - `aspect-loader.handleExtensionLoadingError` (phase `require-aspects`): reached only _after_ the inline compile-and-reload retry is exhausted; and once an aspect is in `failedLoadAspect`, `isAspectLoaded()` returns true so it is never silently re-attempted — it cannot "fail now, succeed later".
  - `resolveInstalledAspectRecursively` (`workspace-aspects-loader.ts`, phase `resolve-installed-aspect`): a synchronous module-not-found on disk — fires only when the package is genuinely absent (→ `bit install`).
  - **Deliberately NOT** the `loadCompsAsAspects` batch catch (`workspace-component-loader.ts`): it can fire for an aspect that loads on a later pass (`clearComponentCache` resets the self-load gate, the load retries), and it cannot attribute the batch throw to a single id — both are false-positive sources. Genuine require failures inside it still surface via `require-aspects`.
