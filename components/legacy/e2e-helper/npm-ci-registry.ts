...
export class NpmCiRegistry {
  ...
  async _addDefaultUser() {
    ...
    // Create local .npmrc file instead of using global config for better npm compatibility
    const npmrcPath = path.join(this.helper.scopes.localPath, '.npmrc');
    const npmrcContent = [
      `${this.ciDefaultScope}:registry=${this.ciRegistry}`,
      `${this.ciRegistry.replace('http://', '//')}:_authToken=${token}`,
      '# Auto-generated by Bit e2e tests',
    ].join('\n');
  ...
  _registerScopes(scopes: string[] = ['@ci']) {
    const npmrcPath = path.join(this.helper.scopes.localPath, '.npmrc');
    const additionalScopes = scopes.filter((scope) => scope !== this.ciDefaultScope);

    if (additionalScopes.length > 0) {
      const additionalContent = additionalScopes.map((scope) => `${scope}:registry=${this.ciRegistry}`).join('\n');
      if (fs.existsSync(npmrcPath)) {
        fs.appendFileSync(npmrcPath, '\n' + additionalContent);
      } else {
        fs.writeFileSync(npmrcPath, additionalContent);
      }
    }
  }
