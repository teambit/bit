...
export class LanesModel {
  ...
  setViewedLane = (viewedLaneId?: LaneId) => {
    this.viewedLane = viewedLaneId ? this.lanes.find((lane) => lane.slug === viewedLaneId.name || lane.id.isEqual(viewedLaneId)) : undefined;
  };

  setViewedOrDefaultLane = (viewedLaneId?: LaneId) => {
    this.viewedLane = viewedLaneId ? this.lanes.find((lane) => lane.slug === viewedLaneId.name || lane.id.isEqual(viewedLaneId)) : this.getDefaultLane();
  };

  resolveComponentFromUrl = (idFromUrl: string, laneId?: LaneId) => {
    const comps = ((laneId && this.lanes.find((lane) => lane.slug === laneId.toString() || lane.id.isEqual(laneId))) || this.viewedLane)?.components || [];
