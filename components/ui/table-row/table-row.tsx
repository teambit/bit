...
import type { GridProps } from '@teambit/base-ui.layout.grid-component';
...
export type CustomRowType = {
  [K in keyof RowType]?: React.ReactNode;
};
  ...
  return (
    ...
    >
      {headings.map((title) => {
        ...
        if (title === 'name') {
          return (
            <TableColumn className={styles.breakWord} key={keyString}>
              <div
        ...
        if (title === 'type') {
          return (
            <TableColumn className={classNames(styles.breakWord, styles.typeColumn)} key={keyString}>
              <div
                className={classNames(styles.mobileTitle, {
                  [styles.show]: isListView,
                })}
        ...
        if (title === 'default') {
          return (
            <TableColumn className={styles.breakWord} key={keyString}>
              ...
              {customRow?.default && <div className={styles.default}>{customRow.default}</div>}
            </TableColumn>
          );
        }
