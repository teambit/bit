...
export default class ContainerVolume {
  ...
  private readArchive(path: PathLike): Promise<Buffer> {
    return this.container.get({ path: path.toString() })
      .then((stream) => {
        let body = '';
  ...
  readFile(path: PathLike, options: { encoding?: null; flag?: string; } | undefined | null, callback: (err?: NodeJS.ErrnoException, data?: Buffer) => void): void {
