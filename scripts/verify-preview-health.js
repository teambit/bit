 ...
 */
const http=require('http');// eslint-disable-next-line import/no-extraneous-dependencies
const WebSocket = require('ws');
const get=p=>new Promise((r,j)=>http.get({host:'127.0.0.1',port:9222,path:p},x=>{let d='';x.on('data',c=>d+=c);x.on('end',()=>r(JSON.parse(d)))}).on('error',j));
const TARGET=Number(process.argv[3]||20), SETTLE=Number(process.argv[4]||30000);
(async()=>{
  ...
  const consoleErrors=[]; const reqs=new Map(); let wire=0;
  const send=(m,q={},sid)=>new Promise(r=>{const i=++id;p.set(i,r);ws.send(JSON.stringify({id:i,method:m,params:q,...(sid?{sessionId:sid}:{})}))});
  const sessions=new Set();
  ws.on('message',async raw=>{const x=JSON.parse(raw);
    if(x.id&&p.has(x.id)){p.get(x.id)(x.result);p.delete(x.id);return}
    const P=x.params||{};
    switch(x.method){
      case 'Target.attachedToTarget':{const sid=P.sessionId;sessions.add(sid);
        await send('Runtime.enable',{},sid);await send('Network.enable',{},sid);
        await send('Runtime.runIfWaitingForDebugger',{},sid);break}
      case 'Runtime.exceptionThrown':
        consoleErrors.push('exception: '+String(P.exceptionDetails?.exception?.description||P.exceptionDetails?.text||'').split('\n')[0].slice(0,160));break;
      case 'Runtime.consoleAPICalled':
        if(P.type==='error')consoleErrors.push('console.error: '+(P.args||[]).map(a=>a.value||a.description||'').join(' ').split('\n')[0].slice(0,160));break;
      case 'Network.requestWillBeSent':
        reqs.set(P.requestId,{url:P.request.url,state:'pending',type:P.type});break;
      case 'Network.responseReceived':{const r=reqs.get(P.requestId);if(r){r.status=P.response.status;}break}
      case 'Network.loadingFinished':{const r=reqs.get(P.requestId);if(r){r.state='done';wire+=P.encodedDataLength||0;}break}
      case 'Network.loadingFailed':{const r=reqs.get(P.requestId);if(r){r.state='failed';r.error=P.errorText;r.canceled=P.canceled;}break}
      default:break;
    }});
  ...
  for(let i=0;i<Math.ceil(SETTLE/250);i++){
    ...
    const r=await send('Runtime.evaluate',{returnByValue:true,expression:`(()=>{const h=document.getElementById('bit-preview-canvas');return h?[...h.children].filter(c=>c.style.visibility==='visible').length:0})()`});
    if(all===null&&(r.result.value||0)>=TARGET){all=((Date.now()-t0)/1000).toFixed(2);}
  ...
  const isExternal=(u)=>{try{return new URL(u).origin!==new URL(process.argv[2]).origin}catch{return true}};
  ...
  const infraErrors=consoleErrors.filter(e=>!/Received .%s. for a non-boolean|cannot be a descendant|cannot contain a nested|without an .onChange.|Each child in a list|validateDOMNesting|closing image tag/.test(e));
  console.log(`frames: ${all??'INCOMPLETE'} s to ${TARGET} | wire ${(wire/1048576).toFixed(1)}MB | requests ${list.length}`);
  console.log(`console errors: ${consoleErrors.length} (infra: ${infraErrors.length}, chunk: ${chunkErrs.length}, content warnings: ${consoleErrors.length-infraErrors.length})`);
