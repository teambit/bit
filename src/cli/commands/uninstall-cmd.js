...
export default class Create extends Command {
  name = 'uninstall <name>';
  description = 'uninstall a transpiler or tester';
  alias = '';
  opts = [];

  action([name, ]: [string], opts: {[string]: boolean}): Promise<*> {
    return new Promise((resolve, reject) => {
      const installedTranpilersGlobingPattern = path.join(TRANSPILERS_DIR, '*');
      ...
      const directoryToRemove = path.join(TRANSPILERS_DIR, name);
