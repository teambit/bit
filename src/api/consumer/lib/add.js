...
export default (async function addAction(
  ...
  function isFileAutoGenerated(consumerPath: string, filePath: string): boolean {
    const fileContent = fs.readFileSync(path.join(consumerPath, filePath)).toString();
  ...
  const addOrUpdateExistingComponentsInBitMap = (
    ...
    const filesWithId = component.files.map((file) => {
      ...
      const bitMapComponentId = !R.isEmpty(foundComponentFromBitMap)
        ...
        : undefined;
      return { id: bitMapComponentId || component.componentId, file };
    });
    const groupFilesById = R.groupBy(componentFromMap => componentFromMap.id);
    const groupedById = groupFilesById(filesWithId);
    return Object.keys(groupedById).map((bitMapComponentId) => {
      ...
      if (foundComponentFromBitMap) {
        component.files = files
          .map((file) => {
            if (!isFileAutoGenerated(consumerPath, file.relativePath)) {
