...
export async function build(id: string, noCache: boolean, verbose: boolean,  workingDir = process.cwd()): Promise<string[] | undefined> {
  const consumer = await loadConsumer(workingDir);
