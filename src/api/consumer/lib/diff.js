...
async function parseValues(
  ...
  if (!isLastItemVersion) {
    return { bitIds: values.map(id => consumer.getBitId(id)) };
  }
  ...
  if (!isOneBeforeLastItemVersion) {
    ...
    return { bitIds: [consumer.getBitId(firstValue)], version: lastValue };
  }
  ...
  return { bitIds: [consumer.getBitId(firstValue)], version: oneBeforeLastValue, toVersion: lastValue };
}
