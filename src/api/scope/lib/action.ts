...
import { Action, ExportValidate, ExportPersist, RemovePendingDir } from '../../../scope/actions';
...
export async function action(scopePath: string, name: string, options: Record<string, any>): Promise<any> {
  ...
  const actionList: ActionClassesList[] = [ExportValidate, ExportPersist, RemovePendingDir];
