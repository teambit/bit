  ...
  describe('addFile', () => {
    describe('dir/file collision', () => {
      it('should not throw when the existing file starts with the added file in the same dir', () => {
        ...
        const addFile = () => dataToPersist.addFile({ path: 'foo/bar.json' });
        expect(addFile).to.not.throw();
      });
      it('should not throw when the added file starts with the existing file in the same dir', () => {
        const dataToPersist = new DataToPersist();
        dataToPersist.addFile({ path: 'foo/bar.json' });
        const addFile = () => dataToPersist.addFile({ path: 'foo/bar.js' });
        expect(addFile).to.not.throw();
      });
      ...
      it('should throw when one is a directory of other with a few levels', () => {
        ...
        dataToPersist.addFile({ path: 'foo1/foo2' });
        const addFile1 = () => dataToPersist.addFile({ path: 'foo1/foo2/foo3/foo4' });
        expect(addFile1).to.throw();

        dataToPersist = new DataToPersist();
        dataToPersist.addFile({ path: 'foo1/foo2/foo3/foo4' });
        const addFile2 = () => dataToPersist.addFile({ path: 'foo1/foo2' });
      ...
      it('should not throw when file are different with the same dir', () => {
        let dataToPersist = new DataToPersist();
        dataToPersist.addFile({ path: 'bar/foo.js' });
        const addFile1 = () => dataToPersist.addFile({ path: 'bar/baz.js' });
        expect(addFile1).to.not.throw();

        dataToPersist = new DataToPersist();
        dataToPersist.addFile({ path: 'bar/baz.js' });
        const addFile2 = () => dataToPersist.addFile({ path: 'bar/foo.js' });
        expect(addFile2).to.not.throw();
      });
    });
  });
