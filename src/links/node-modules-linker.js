...
export default class NodeModuleLinker {
   ...
   * for the rest, it's populated here
   */
  async _populateShouldDependenciesSavedAsComponentsData(): Promise<void> {
    if (!this.components.length) return;
    // don't repopulate if they were entered already, such as during bit import
    const oneOfTheComponents = this.components[0];
    if (typeof oneOfTheComponents.dependenciesSavedAsComponents !== 'undefined' || !this.consumer) return;
